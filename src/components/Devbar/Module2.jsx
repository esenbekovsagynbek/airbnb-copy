  ...
  return (
    <div>
      ...
      <p>
        ...
        обработчиками событий, чтобы позволить нашим пользователям
      ...
      <p>
        ...
        приложение, сделав его с состоянием. Состояние позволяет вещам
        изменяться со временем и является очень важной частью любого приложения.
        А чтобы изменить состояние, нам нужно будет научиться работать с
      ...
      <p>
        Нам нужно будет создать новый компонент под названием{' '}
        <code>ListingFilters</code> который будет обрабатывать фильтры и
        отображать некоторые компоненты, позволяющие пользователям фильтровать
        объявления, ища их название, выбирая диапазон дат и добавляя количество
        гостей. Затем мы добавим этот компонент на <code>HomePage</code> и
        используем его для фильтрации объявлений в компоненте{' '}
        <code>HomePage</code>.
      </p>
      <h3>Задачи</h3>
      <Separator className='mb-2' />
  ...
  return (
    <div>
      ...
      <p>
        ...
        состоянием. В настоящее время мы импортируем <code>listings</code>{' '}
        напрямую из файла <code>src/api/data/listings</code>, что означает, что{' '}
        <code>listings</code> статичны и никогда не изменятся.
      </p>
      <p>
        Для этого мы воспользуемся одним из самых распространенных хуков в
        React: <code>useState</code>. Этот хук позволит нам хранить переменную
        состояния, а также предоставит нам функцию обновления для ее изменения.
        Мы можем вызвать эту функцию, когда захотим обновить наше состояние.
      </p>
      <p>
        Нам нужно будет использовать псевдоним при импорте наших listings,
        поскольку теперь у нас будет новая переменная состояния с именем{' '}
        <code>listings</code>, и мы не можем иметь дубликатов. Чтобы упростить
        задачу, мы назовем псевдоним <code>staticListings</code>. Затем мы можем
        использовать хук <code>useState</code>, чтобы создать переменную
        состояния с именем <code>listings</code> и передать ей эти статические
        listings.
      </p>
      <p>
        С этими изменениями наше приложение будет работать так же, и не будет
        никаких явных различий. Однако теперь мы позволили нашим{' '}
        <code>listings</code> динамически изменяться со временем, что позволит
        нам добавить некоторую функциональность для изменения отображаемых
        listings позже.
      </p>
      <p>
...
export const Step2 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда наши <code>listings</code> находятся в состоянии в{' '}
        <code>HomePage</code>, мы можем начать создавать компоненты, которые
        будут их изменять. Нам нужно будет добавить строку поиска, чтобы наши
        пользователи могли искать объявления по заголовку, выбор дат, чтобы
        пользователи могли выбрать свои даты, ввод с шагами, чтобы пользователи
        могли выбрать количество гостей, и, наконец, нам понадобится кнопка
        отправки, чтобы установить фильтры и обновить объявления.
      </p>
      <p>
        Лучший способ сделать это — создать новый компонент, который мы назовем{' '}
        <code>ListingFilters</code>. Создание нового компонента для этого —
        хорошая идея, потому что мы можем инкапсулировать всю логику и интерфейс
        для фильтров в одном месте, а затем просто использовать этот компонент в{' '}
        <code>HomePage</code>.
      </p>
      <p>
        Здесь мы также будем использовать несколько полезных компонентов из
        папки <code>src/components/ui</code>. У нас есть компонент{' '}
        <code>Input</code>, компонент <code>DateRangePicker</code>, компонент{' '}
        <code>Stepper</code> и компонент <code>Button</code>. Мы также
        используем иконку <code>Search</code> из <code>lucide-react</code>.
      </p>
      <p>
        Нам нужно будет создать новый файл в директории{' '}
        <code>src/components</code> под названием{' '}
        <code>ListingFilters.jsx</code> со следующим кодом:
      </p>
...
export const Step3 = () => {
  return (
    <div>
      <h2>
        Добавление состояния и обработчиков событий в{' '}
        <code>ListingFilters</code>
      </h2>
      <p>
        Наш компонент <code>ListingFilters</code> в данный момент ничего не
        делает, кроме как отображает интерфейс. Нам нужно изменить это и
        добавить в него состояние и обработчики событий, чтобы мы могли
        использовать его для фильтрации наших объявлений.
      </p>
      <p>
        Нам нужно будет создать 3 переменные состояния: <code>dates</code>,{' '}
        <code>guests</code> и <code>search</code>. Мы передадим{' '}
        <code>dates</code> в <code>DateRangePicker</code>, <code>guests</code> в{' '}
        <code>Stepper</code>, а <code>search</code> в компонент{' '}
        <code>Input</code>. Мы также передадим функции обновления этим
        компонентам, чтобы они могли обновлять переменные состояния.
      </p>
      <p>
        ...
        <code>setDates</code>, так как он уже настроен на возврат правильных
        данных. Даты будут объектом с свойствами <code>from</code> и{' '}
        <code>to</code>.
      </p>
      <p>
        Кроме того, нам нужно добавить свойство <code>minDate</code> к{' '}
        <code>DateRangePicker</code> со значением текущей даты, чтобы не
        позволить пользователям выбирать даты в прошлом! Мы не будем
        ограничивать даты в будущем, так что пользователи могут бронировать на
        любое будущее время.
      </p>
      <p>
...
export const Step4 = () => {
  return (
    <div>
      ...
      <p>
        Наш компонент <code>ListingFilters</code> выглядит хорошо, но его
        состояние является самодостаточным. В данный момент у нас нет способа
        передать это состояние в компонент <code>HomePage</code>, где оно нам
        нужно.
      </p>
      <p>
        Важно понять, что <code>HomePage</code> отвечает за фильтрацию{' '}
        <code>listings</code>, а не компонент <code>ListingFilters</code>. Это
        ответственность <code>HomePage</code>, поскольку именно он хранит
        состояние для <code>listings</code>. <code>ListingFilters</code>
        просто будет управлять своим внутренним состоянием, за которое отвечает
        сам, и только вызывать колбэк из <code>HomePage</code> с фильтрами.
      </p>
      <p>
        Чтобы передать состояние вверх, нам нужно будет два вещи. Сначала нам
        нужна <code>onChange</code> колбэк-функция, которую{' '}
        <code>HomePage</code> передаст <code>ListingFilters</code>, а затем нам
        нужна функция <code>handleSubmit</code>, которую{' '}
        <code>ListingFilters</code>
        вызовет, когда будет нажата <code>Button</code>. Функция{' '}
        <code>handleSubmit</code>
        затем вызовет колбэк <code>onChange</code>.
      </p>
      <p>
        Функция <code>handleSubmit</code> будет вызывать колбэк{' '}
        <code>onChange</code> с текущими значениями <code>dates</code>,{' '}
        <code>guests</code> и <code>search</code>. Мы прикрепим её к{' '}
        <code>Button</code>, чтобы каждый раз, когда она нажимается, мы
        передавали состояние вверх для использования в <code>HomePage</code>.
      </p>
      <p>
        Нам нужно будет обновить компонент <code>ListingFilters</code> следующим
        кодом:
  ...
  return (
    <div>
      ...
      <p>
        ...
        функционален, мы можем интегрировать его в <code>HomePage</code>. Нам
        нужно будет импортировать <code>ListingFilters</code> в{' '}
        <code>HomePage</code> и отобразить его.
      </p>
      <p>
        Пока не будем беспокоиться о передаче колбэка <code>onChange</code>, так
        как мы создадим его на следующем шаге. Но нам нужно будет позаботиться о
        расположении и стилях <code>ListingFilters</code>, чтобы он хорошо
        смотрелся в <code>HomePage</code>.
      </p>
      <p>
        Мы разместим <code>ListingFilters</code> вверху страницы и используем{' '}
        <code>Separator</code> из нашей папки <code>src/components/ui</code>,
        чтобы создать визуальное отделение между ним и остальной частью
        страницы. Это сделает наши фильтры видимыми в верхней части экрана, где
        пользователи их ожидают.
      </p>
      <p>
        Нам нужно будет обновить компонент <code>HomePage</code> следующим
        кодом:
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>ListingFilters</code> как его пропс <code>onChange</code>. Как мы
        уже видели, этот колбэк будет вызываться каждый раз, когда фильтры
  ...
  return (
    <div className='relative'>
      ...
      <p>
        ...
        нажмите кнопку для отправки ваших фильтров. Это отфильтрует ваши списки
        и покажет только те, которые соответствуют выбранным вами критериям!
      </p>
      <p>
        ...
        компонентами и как использовать их для обновления состояния в
        родительском компоненте. Мы работали с обработчиками событий и научились
        использовать их для получения значения в ответ на ввод пользователя.
      </p>
      <p>
