...
export const Intro = () => {
  return (
    <div>
      ...
      <p>
        Мы будем использовать популярную библиотеку Redux для этой цели, которая
        отлично подходит для написания масштабируемого управления состоянием. Мы
        будем использовать ее вместе с Redux Toolkit, который предоставляет
        множество функциональных возможностей «из коробки», чтобы мы могли
        сосредоточиться на том, что действительно важно: создании отличного
        приложения!
      </p>
      <h3>Описание</h3>
      <Separator className='mb-2' />
      <p>
        В данный момент все наши данные хранятся локально в состоянии наших
        компонентов. Например,
        <code>listings</code> в <code>HomePage</code> загружаются и хранятся в
        локальном состоянии
        <code>HomePage</code> через кастомный хук <code>useFetch</code>. То же
        самое справедливо и для компонента <code>ListingDetailsPage</code>.
      </p>
      <p>
        Нам нужно изменить это, потому что мы добавим новую функциональность в
        наше приложение. Мы добавим новую страницу «Избранное», которая будет
        отображать избранные объявления пользователя. Поскольку у нас будет
        несколько страниц, которые могут отображать одно и то же объявление,
        добавление объявления в избранное должно отражаться на любой странице,
        где это объявление находится. Это потребует от нас наличия глобального
        состояния.
      </p>
      <p>
        Нам нужно будет использовать Redux для хранения списков в глобальном
        состоянии, а затем сделать их доступными для любого компонента в нашем
        приложении. Это позволит нам затем обновить списки, которые добавлены в
        избранное пользователем, и динамически показывать их на странице
        «Избранное».
      </p>
      <p>
        Важно отметить, что мы будем обрабатывать все это только на стороне
        клиента. Поскольку у нас нет бэкенда, нет способа сохранить объявления в
        избранном. Вместо этого мы будем хранить это состояние в памяти и
        использовать его, чтобы показать, является ли объявление избранным или
        нет. Но как только мы обновим страницу, это состояние будет сброшено.
      </p>
      <p>
        Чтобы сделать все это, нам нужно будет настроить Redux, создать наше
        глобальное хранилище, а также создать редюсеры и действия, которые будут
        контролировать, как хранилище изменяется со временем. Затем мы изменим
        нашу загрузку данных, чтобы она выполнялась через Redux, а также
        создадим страницу «Избранное», чтобы пользователи могли добавлять любое
        объявление в избранное.
      </p>
      <h3>Задачи</h3>
      <Separator className='mb-2' />
      <ul>
...
export const Step1 = () => {
  return (
    <div>
      ...
      <p>
        Первое, что нам нужно сделать, это настроить Redux хранилище. Redux
        хранилище является точкой входа в состояние Redux. Мы будем использовать
        его для доступа ко всем нашим индивидуальным редюсерам по всему
        приложению.
      </p>
      <p>
        Хранилище обычно состоит из нескольких редюсеров, каждый из которых
        представляет собой «кусочек» приложения, связанный с определенными
        функциями. Пока что, поскольку у нас еще нет созданных редюсеров, мы
        просто предоставим пустой объект. Нам нужно экспортировать это
        хранилище, так как мы будем использовать его на следующем этапе.
      </p>
      <p>
        Мы создадим новый файл в папке <code>src/state</code> с именем{' '}
        <code>store.js</code> со следующим кодом:
      </p>
...
export const Step2 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда у нас есть наше хранилище, нам нужно подключить его к
        React, поскольку наше React-приложение не может напрямую
        взаимодействовать с Redux. На самом деле, Redux вообще не требует React
        для работы; он полностью автономен! Чтобы получить доступ к хранилищу,
        нам нужно подключить его к нашему React-приложению через{' '}
        <code>Provider</code> из <code>react-redux</code>.
      </p>
      <p>
        <code>Provider</code> из <code>react-redux</code> работает через Context
        API от React. Мы можем обернуть наше приложение в этот компонент, как мы
        делаем с любым другим контекстом, и затем передать ему{' '}
        <code>store</code>, которое мы создали на предыдущем этапе. Это сделает
        наше
  ...
  return (
    <div>
      ...
      <p>
        ...
        помещается в наше <code>store</code>, чтобы быть доступным в нашем
        приложении.
      </p>
      <p>
      ...
      <p>
        ...
        <code>listings</code>, затем передадим наш <code>initialState</code>, а
        затем определим пустой объект для <code>reducers</code>, так как мы
        будем их создавать на следующих этапах. Наконец, мы экспортируем{' '}
        <code>listingsSlice.reducer</code>, чтобы использовать в нашем
        хранилище.
      </p>
      <p>
        Нам нужно создать новый файл внутри <code>src/state/listings</code> под
        названием <code>listingsSlice.js</code> с следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        импортировать редьюсер, который мы экспортировали, и подключить его к
        нашему <code>store</code>. Это автоматически даст нашему React
        приложению доступ к этому срезу.
      </p>
      <p>
        Преимущество такого подхода заключается в том, что мы можем создать
        столько срезов, сколько захотим, и импортировать их сюда, чтобы
        объединить их в нашем хранилище. Это позволит нам иметь разделение
        обязанностей и эффективно организовать наше приложение по мере его
        роста, но в то же время иметь одну центральную точку для нашего Redux
        состояния, доступного из любой части нашего приложения.
      </p>
      <p>
...
export const Step5 = () => {
  return (
    <div>
      ...
      <p>
        Следующий шаг — начать обрабатывать получение <code>listings</code>{' '}
        через Redux. Мы будем делать то же самое, что и раньше, с единственным
        отличием, что теперь мы будем использовать <code>listingsSlice</code>{' '}
        для хранения состояния и его обновления, вместо того чтобы хранить его
        локально в компоненте <code>HomePage</code>.
      </p>
      <p>
        Для этого нам нужно использовать <code>createAsyncThunk</code> из Redux,
        чтобы создать асинхронное действие. Мы дадим ему имя{' '}
        <code>listings/fetchListings</code>, чтобы обозначить срез и действие, а
        затем передадим ему асинхронную функцию, которая будет получать списки с
        того же конечного пункта, что и раньше. Нам также нужно будет сделать
        так, чтобы эта функция принимала необязательный параметр{' '}
        <code>options</code>, как это было в <code>useFetch</code>
        ранее, чтобы сохранить ту же функциональность фильтрации.
      </p>
      <p>
        Важно отметить, что поскольку мы используем{' '}
        <code>createAsyncThunk</code> из Redux Toolkit, нам не нужно явно
        обрабатывать состояния ошибки и загрузки внутри этой функции. Вместо
        этого мы обработаем их на следующем этапе, добавив некоторые редьюсеры.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        сохранить полученные <code>listings</code> в состоянии, а также
        обработать состояния ошибки и загрузки. К счастью, большая часть этого
      ...
      <p>
        ...
        содержащим наши данные. Мы обновим свойство <code>status</code> во время
        получения, затем, если возникнет ошибка, установим ее в
        <code>rejected</code> состояние, иначе обновим <code>listings</code>{' '}
        данными из нашего API.
      </p>
      <p>
...
export const Step7 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда наш срез готов с действиями и редюсерами, мы можем наконец
        начать использовать его в наших компонентах React. Мы начнем с
        рефакторинга <code>HomePage</code>, чтобы теперь получать{' '}
        <code>listings</code> с помощью нашего нового асинхронного действия, и
        мы получим состояние из Redux с помощью селектора.
      </p>
      <p>
        Чтобы получить любую часть состояния в Redux, вы используете селектор
        через <code>useSelector</code>. Селектор — это функция, которая
        принимает
        <code>state</code> и выбирает любую его подмножество. В нашем случае мы
        хотим получить доступ к <code>listingsSlice</code>, который будет
        находиться под <code>state.listings</code>. Передав этот селектор в{' '}
        <code>useSelector</code>, мы получим ту часть состояния, которая хранит
        наши <code>listings</code>, <code>error</code> и <code>status</code>{' '}
        состояния.
      </p>
      <p>
        Затем, чтобы отправлять действия в Redux, мы будем использовать функцию
        <code>dispatch</code>, возвращаемую из <code>useDispatch</code>. Мы
        сможем отправить действие <code>fetchListings</code> с его помощью, и
        Redux автоматически обработает обновление состояния за нас.
      </p>
      <p>
        Поскольку мы больше не используем пользовательский хук{' '}
        <code>useFetch</code>
        ...
        вызвать в функции очистки <code>useEffect</code>. Наш запрос теперь
        будет правильно обработан во время отмены с помощью кода, который мы
        настроили здесь и на предыдущем этапе.
      </p>
      <p>
        ...
        реализацию собственного решения для кеширования, как мы сделали ранее,
        так что мы оставим это без изменений.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        приложение. Мы добавим возможность пользователям отмечать свои
        объявления как избранные и отображать их на отдельной странице. Мы будем
        использовать
      ...
      <p>
        ...
        Оно будет хранить ID избранных объявлений пользователя. Мы будем
        использовать эти ID, чтобы проверить, является ли объявление избранным
        или нет.
      </p>
      <p>
        Затем нам нужно будет создать редюсеры для обработки добавления и
        удаления объявления из избранного. Это будут синхронные действия,
        поэтому нам не нужно будет использовать асинхронный thunk. Вместо этого
        мы определим два редюсера внутри объекта <code>reducers</code> нашего
        среза.
      </p>
      <p>
        Поскольку мы работаем с Redux Toolkit и используем{' '}
        <code>createSlice</code>, нам не нужно создавать соответствующие
        действия, как мы делали с асинхронным thunk. Вместо этого мы можем
        просто получить и экспортировать их через{' '}
        <code>listingsSlice.actions</code>.
      </p>
      <p>
...
export const Step9 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда мы настроили наш срез для работы с избранными
        объявлениями, мы можем приступить к созданию страницы для них. Это будет
        страница "Избранное", которая будет отвечать за получение избранных
        объявлений из <code>listingsSlice</code> и их отображение.
      </p>
      <p>
        Мы будем использовать хук <code>useSelector</code>, чтобы получить
        доступ к<code>state.listings</code>, что даст нам доступ к{' '}
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>Navbar</code> в наш компонент <code>App</code>. Это позволит нам
        видеть навигационную панель на каждой странице нашего приложения.
      </p>
      <p>
        Однако мы не хотим, чтобы <code>Navbar</code> перекрывал{' '}
        <code>Devbar</code>, поэтому мы добавим его сразу над компонентом{' '}
        <code>Outlet</code>, чтобы он находился в тех же стилях контейнера и не
        мешал левой части приложения.
      </p>
      <p>
...
export const Step13 = () => {
  return (
    <div>
      ...
      <p>
        Поскольку это кнопка, которую мы будем использовать в нескольких местах,
        разумно сделать ее переиспользуемым компонентом. Поскольку у нас есть
        доступ к Redux из любого места в нашем приложении, также будет логично,
        чтобы этот компонент инкапсулировал всю логику, необходимую для
  ...
  return (
    <div>
      ...
      <p>
        ...
        изображения. Мы будем использовать абсолютное позиционирование для
        этого, чтобы она была четко видна.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        тому же состоянию через хуки <code>useSelector</code> и обновляет то же
        состояние через вызовы <code>dispatch</code>. Существует только один
...
export const Completed = () => {
  return (
    <div className='relative'>
      ...
      <p>
        Поздравляем! Вы успешно завершили 6-й модуль курса. Теперь у вас должны
        быть данные из Redux store, а также возможность добавлять и удалять
        объявления из избранного, чтобы они отображались на странице
        «Избранное»!
      </p>
      <p>
        В этом модуле вы узнали, как настроить и использовать Redux, как создать
        хранилище, действия и редюсеры, а также как соединить все это из разных
        компонентов. Вы перенесли получение объявлений в Redux и добавили
