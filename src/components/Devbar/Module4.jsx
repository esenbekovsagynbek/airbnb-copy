  ...
  return (
    <div>
      ...
      <p>
        ...
        в наше приложение, затем создадим маршрутизатор, который будет содержать
        наши маршруты и структуру навигации для нашего приложения, и в конце
        добавим новые страницы, на которые смогут переходить пользователи.
      </p>
      <p>
        Этот модуль очень важен, поскольку мы кардинально изменим структуру
        нашего приложения. Мы заменим точку входа нашего приложения, которая в
        настоящее время рендерит один компонент, на рендеринг целого
        маршрутизатора!
      </p>
      <h3>Описание</h3>
      <Separator className='mb-2' />
      <p>
        В настоящее время все наши объявления не кликабельны. Мы показываем их
        все на главной странице и даже позволяем пользователю фильтровать их, но
        не даем возможности нажать на объявление, чтобы просмотреть
        дополнительную информацию о нем. Это не лучший пользовательский опыт.
      </p>
      <p>
        Мы можем улучшить это, создав новую страницу, которая будет отображать
        детали объявления. Пользователь сможет кликнуть на любое объявление и
        быть перенаправленным на эту страницу, а затем, когда они закончат, они
        должны иметь возможность вернуться на главную страницу через свой
        браузер. Это создаст гораздо лучший пользовательский опыт и сделает наше
        приложение более живым.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        определим наши маршруты и их структуру. Это станет новой точкой входа в
        наше приложение.
      </p>
      <p>
      ...
      <p>
        Нам нужно будет создать новый файл в директории <code>src</code>с именем{' '}
        <code>Router.jsx</code> и следующим кодом:
      </p>
...
export const Step2 = () => {
  return (
    <div>
      ...
      <p>
        С этими изменениями наш <code>Router</code> теперь контролирует все
        приложение и решает, что будет рендериться. Поскольку мы поместили
...
export const Step3 = () => {
  return (
    <div>
      ...
      <p>
        Следующее, что нам нужно сделать, это позволить <code>App</code>{' '}
        рендерить динамическое содержимое. В текущем состоянии <code>App</code>{' '}
        не способен рендерить динамическое содержимое. Мы жестко закодировали{' '}
        <code>HomePage</code> внутри него. Мы должны изменить это, чтобы
        позволить рендерить любой компонент в зависимости от текущего URL. Мы
        также хотим сохранить тот же стиль, что и сейчас, а также рендерить
      ...
      <p>
        ...
        является возможность иметь дочерние маршруты. Это позволит нам
        определить маршрут, который является дочерним к другому маршруту. Когда
        мы будем перемещаться к этому маршруту, будут рендериться как
        родительский, так и дочерний маршрут. Мы можем использовать{' '}
        <code>App</code> в качестве родительского маршрута и любой динамический
        маршрут в качестве дочернего.
      </p>
      <p>
        Для этого нам нужно будет добавить новый маршрут внутри{' '}
        <code>Router</code>, под свойством <code>children</code> индексового
        маршрута. Мы дадим ему тот же путь, <code>/</code>, и будем использовать
        компонент <code>HomePage</code>. Это рендерит все так, как у нас есть
        сейчас, но через дочерний маршрут в <code>Router</code>.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>App</code>, нам нужно сказать <code>react-router-dom</code>, где
        рендерить дочерний маршрут.
      </p>
      <p>
        Для этого мы воспользуемся компонентом <code>Outlet</code> из{' '}
        <code>react-router-dom</code>. Этот компонент является местом, где{' '}
        <code>react-router-dom</code> будет динамически рендерить маршрут внутри
        содержимого другого маршрута. В нашем случае он будет рендерить любого
        потомка <code>App</code>.
      </p>
      <p>
        Мы заменим жестко закодированный <code>HomePage</code> на компонент{' '}
        <code>Outlet</code>, чтобы сохранить весь тот же стиль, что и прежде, а
        также чтобы <code>Devbar</code> был виден в любое время.
      </p>
      <p>
...
export const Step5 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда наш маршрутизатор настроен и работает с вложенными
        маршрутами, мы можем начать добавлять новые страницы в наше приложение.
        Мы уже говорили о добавлении страницы с деталями объявления, чтобы
        пользователь мог перейти к ней и увидеть больше информации о списке, так
        что давайте сделаем это!
      </p>
      <p>
        Первое, что нам нужно сделать, это создать компонент, чтобы показать
        некоторые детали объявления. Это будет очень похоже на компонент{' '}
        <code>ListingCard</code>, который мы создали ранее, но у него будет
        больше информации для отображения. Он также будет получать{' '}
        <code>listing</code> в качестве пропсов и просто рендерить интерфейс для
        объявления.
      </p>
      <p>
        Сначала мы начнем без изображений. Мы просто будем рендерить название{' '}
        <code>listing</code>, цену, местоположение, максимальное количество
        гостей и описание. На следующих шагах мы добавим изображения, так же как
        и в <code>ListingCard</code>.
      </p>
      <p>
        Нам нужно создать новый файл внутри <code>src/components</code> с именем{' '}
        <code>ListingDetailsCard.jsx</code> со следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        компонент страницы, который мы подключим к нашему <code>router</code>, и
        он будет рендерить компонент <code>ListingDetailsCard</code>, который мы
        только что создали на предыдущем шаге.
      </p>
      <p>
        Поскольку наш компонент <code>ListingDetailsCard</code> нуждается в{' '}
        <code>listing</code> для отображения, нам нужно будет получить его на
        этой странице и передать дальше. Так же, как мы делали на{' '}
        <code>HomePage</code>, нам нужно будет получить объявление в{' '}
        <code>useEffect</code>, а также обработать состояния загрузки и ошибки и
        управлять условиями гонки.
      </p>
      <p>
        Чтобы получить правильное объявление, нам нужно знать{' '}
        <code>listingId</code>, чтобы его получить. Конечная точка API для
        получения одного объявления требует его. Мы можем использовать параметры
        ...
        <code>listingId</code> в этот маршрут, когда мы будем к нему переходить.
      </p>
      <p>
        Чтобы получить доступ к <code>listingId</code> и любым другим
        параметрам, мы можем использовать хук <code>useParams</code> из{' '}
        <code>react-router-dom</code>. Это даст нам доступ к любым параметрам,
        которые были переданы в этот маршрут.
      </p>
      <p>
        Нам нужно создать новый файл внутри <code>src/pages</code> с именем{' '}
        <code>ListingDetailsPage.jsx</code> со следующим кодом:
      </p>
...
export const Step7 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда мы создали <code>ListingDetailsPage</code>, мы можем
        добавить его в наш <code>router</code>, чтобы он был доступен в нашем
        приложении. Мы захотим отобразить этот маршрут как дочерний маршрут
        нашего индексного маршрута, прямо рядом с <code>HomePage</code>. Это
        гарантирует, что навигация происходит только между дочерними маршрутами,
        при этом <code>App</code> остается отображенным с компонентом{' '}
        <code>Devbar</code>.
      </p>
      <p>
        Путь для страницы деталей объявления будет содержать динамический
        параметр URL, <code>listingId</code>. Путь будет{' '}
        <code>/listings/:listingId</code>, который будет соответствовать любому{' '}
        <code>listingId</code>, который мы предоставим. Таким образом, мы
        получим доступ к <code>listingId</code> через параметры маршрута,
        которые мы получили с помощью <code>useParams</code> на предыдущем шаге.
      </p>
      <p>
        Важно понимать взаимосвязь между различными маршрутами и их путями. В
        настоящее время мы поместили <code>ListingDetailsPage</code> под
        основной индексный путь <code>/</code>, где находится <code>App</code>,
        и затем указали <code>/listings/:listingId</code> как путь. Первый путь
        применяется к компоненту <code>App</code>, который должен отображаться
        на любом маршруте, так как он содержит основной макет нашего приложения.
        Компонент <code>ListingDetailsPage</code>, однако, должен отображаться
        только на <code>/listings/:listingId</code>. В результате на каждой
        странице будет отображаться <code>App</code>, а любой подмаршрут,
        который мы определили, будет отображать соответствующий компонент внутри{' '}
        <code>Outlet</code> компонента <code>App</code>.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>listing</code> внутри <code>ListingCard</code>, мы можем
        использовать это для передачи правильного <code>listingId</code> в URL.
      </p>
      <p>
        ...
        ссылки. Это позволит нам реализовать маршрутизацию на стороне клиента,
        что не перезагрузит страницу, как это сделает обычный тег <code>a</code>
        . Компонент <code>Link</code> принимает свойство <code>to</code>,
        которое соответствует <code>href</code> в обычной ссылке. Мы можем
        передать относительную ссылку и использовать интерполяцию строк для
        передачи <code>listingId</code>.
      </p>
      <p>
        Вы заметите, что мы решили сделать так, чтобы <code>Link</code>{' '}
        оборачивал весь компонент <code>ListingCard</code>. Это сделано для
        того, чтобы вся карточка становилась кликабельной, как вы и ожидали бы в
        нормальном приложении. Однако мы могли бы сделать это и иначе, например,
        создав кнопку и сделав кликабельной только ее. В любом случае это
        нормально, и это полностью вопрос предпочтений на этом этапе.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        на любое объявление, чтобы перейти на страницу его деталей. Теперь
      ...
      <p>
        ...
        <code>ListingCardImages</code>, но также будет отслеживать текущий
        индекс изображения и отображать изображение в большем размере. Это
        позволит пользователю кликнуть на любое изображение и увидеть его в
        большом размере, а также просматривать изображения, используя карусель.
      </p>
      <p>
      ...
      <p>
        Нам нужно создать новый файл внутри <code>src/components</code> с именем{' '}
        <code>ListingDetailsCardImages.jsx</code> со следующим кодом:
      </p>
...
export const Step10 = () => {
  return (
    <div>
      ...
      <p>
        Нам нужно импортировать компонент <code>ListingDetailsCardImages</code>и
        отобразить его внутри компонента <code>ListingDetailsCard</code>,
      ...
      <p>
        Нам нужно обновить компонент <code>ListingDetailsCard</code> со
        следующим кодом:
      </p>
      <CodeHighlighter
...
export const Step11 = () => {
  return (
    <div>
      ...
      <p>
        Способ, которым мы можем это исправить с помощью{' '}
        <code>react-router-dom</code>, заключается в том, что нам нужно
        предоставить резервный компонент на случай, если текущий URL не
        соответствует ни одному действительному маршруту, который в настоящее
        время только <code>/</code> и <code>/listings/:listingId</code>. Мы
        можем создать общий компонент страницы <code>NotFoundPage</code>, чтобы
        отобразить некоторый пользовательский интерфейс для пользователей,
        которые попадают на эту страницу.
      </p>
      <p>
        На этой странице мы также должны добавить кнопку для возврата на
        домашнюю страницу, чтобы облегчить пользователям поиск пути обратно. Мы
        снова используем компонент <code>Link</code>, чтобы отправить наших
        пользователей на домашнюю страницу. Мы также передадим{' '}
        <code>replace</code> в качестве пропса для <code>Link</code>, чтобы вся
        история заменилась и пользователи не могли вернуться на страницу ошибки.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        нужно подключить его к нашему <code>router</code>, чтобы он служил
        запасным вариантом для любого маршрута, который не обрабатывается. Для
        этого мы воспользуемся свойством <code>errorElement</code>.
      </p>
      <p>
      ...
      <p>
        ...
        ошибки на любом маршруте. После этого мы сможем ввести любой
        недействительный URL, и мы всегда увидим этот компонент, что является
        гораздо лучшим пользовательским опытом.
      </p>
      <p>
...
export const Completed = () => {
  return (
    <div className='relative'>
      ...
      <p>
        Поздравляем! Вы успешно завершили 4-й модуль курса. Теперь вы можете
        перемещаться между главной страницей и страницей деталей списка, щелкая
        по любому списку!
      </p>
      <p>
        В этом модуле мы узнали, как работать с <code>react-router-dom</code>,
        как создать маршрутизатор с помощью <code>createBrowserRouter</code> и
        передать его в наше приложение через <code>RouterProvider</code>, мы
        узнали, как перемещаться между маршрутами и передавать параметры, а
        также научились создавать наш собственный компонент "страница не
        найдена" для отображения.
      </p>
      <p>
