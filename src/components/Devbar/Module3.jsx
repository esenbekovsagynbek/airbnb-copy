  ...
  return (
    <div>
      ...
      <p>
        ...
        изучим <code>useEffect</code> и его работу, узнаем о массивах
        зависимостей, а также о жизненном цикле эффекта в компоненте React.
      </p>
      <p>
        Мы также научимся получать данные из API, используя{' '}
        <code>useEffect</code>. Мы узнаем, как управлять своими состояниями
        загрузки и ошибок, а также как предотвратить гонки условий при
        нескольких запросах, выполняемых в разное время!
      </p>
      <h3>Описание</h3>
      <Separator className='mb-2' />
      <p>
        В настоящее время, хотя наши <code>listings</code> в{' '}
        <code>HomePage</code>
        ...
        только статический список, импортированный ранее. Нам нужно это
        изменить.
      </p>
      <p>
      ...
      <p>
        Важно отметить, что <code>api</code>, доступный в нашем приложении,
        является мок API. Это значит, что мы будем работать с ним и вызывать
        его, как в реальном приложении, но все данные будут смоделированы.
        Данные будут вместо этого поступать из <code>localStorage</code>,
        который инициализируется из того же статического списка{' '}
        <code>listings</code>, который мы использовали до сих пор. Мы делаем
        это, потому что у нас нет бэкенда для этого приложения, поэтому мы
        вместо этого создаем его в виде мока. Но это замечательно, потому что мы
...
export const Step1 = () => {
  return (
    <div>
      ...
      <p>
        Первое, что нам нужно сделать, это удалить <code>staticListings</code>{' '}
        из <code>HomePage</code>, так как мы больше не будем использовать их
        напрямую. Мы подготовим <code>HomePage</code> для получения{' '}
        <code>listings</code> из мок API. Затем мы установим начальное состояние{' '}
        <code>listings</code> как пустой массив. Это приведет к исчезновению{' '}
        <code>listings</code> из UI на данный момент, но не беспокойтесь, мы
        добавим их обратно на следующем этапе.
      </p>
      <p>
        ...
        реализуем фильтрацию, отправив данные в API. На данный момент мы оставим
        функцию <code>handleFilters</code> пустой с комментарием, чтобы
  ...
  return (
    <div>
      ...
      <p>
        ...
        реализовать функцию для получения списков. Мы определим эту функцию
        внутри
        <code>useEffect</code> и вызовем ее, когда <code>HomePage</code> будет
        смонтирован. Мы назовем ее <code>fetchListings</code>, чтобы было
      ...
      <p>
        ...
        чтобы получить все списки. Это стандартный способ получения данных из
        API, и наш мок <code>api</code> позволяет делать это так, как будто мы
        получаем данные из реального API.
      </p>
      <p>
        Как только мы получим ответ от API, мы обновим <code>listings</code>,
        вызвав функцию <code>setListings</code> с свойством{' '}
        <code>response.data</code>. Это перерисует наш компонент и покажет
        полученные списки на экране.
      </p>
      <p>
        Нам нужно обновить компонент <code>HomePage</code> с помощью следующего
        кода:
...
export const Step3 = () => {
  return (
    <div>
      ...
      <p>
        Отлично! Наши списки теперь извлекаются из API и корректно отображаются
        на главной странице. Однако есть небольшая ошибка в пользовательском
        интерфейсе. На данный момент пользователь видит сообщение "Нет списков."
        до тех пор, пока запрос не вернет данные, что не является хорошим
        пользовательским опытом. Нам нужно сделать что-то, чтобы сообщить
        пользователю, что списки загружаются.
      </p>
      <p>
        Для этого нам нужно создать и отслеживать собственное состояние
        загрузки. Мы сделаем это с помощью переменной, называемой{' '}
        <code>isLoading</code>. Мы установим ее в <code>true</code> изначально,
        когда наш компонент будет смонтирован, а затем в <code>false</code>,
        когда запрос вернет данные. Поскольку мы будем разрешать множественные
        запросы во время жизненного цикла этого компонента, нам также нужно
        будет убедиться, что мы снова устанавливаем <code>isLoading</code> в{' '}
        <code>true</code> каждый раз, когда вызывается функция{' '}
        <code>fetchListings</code>.
      </p>
      <p>
        ...
        хотим отобразить. Мы можем настроить ее так, чтобы она возвращала
        компонент <code>Spinner</code> пока <code>isLoading</code> равно{' '}
...
export const Step4 = () => {
  return (
    <div>
      ...
      <p>
        Чтобы сделать это, нам нужно переместить наш код извлечения данных
        внутрь блока try-catch, чтобы мы могли поймать ошибку, если что-то
        пойдет не так. Нам также нужно помнить о сбросе состояния ошибки каждый
        раз, когда мы вызываем функцию <code>fetchListings</code>, так как новый
        запрос должен очищать предыдущую ошибку. Если произойдет ошибка, мы
        просто установим сообщение "Что-то пошло не так. Пожалуйста, попробуйте
        позже." в состояние <code>error</code>.
      </p>
      <p>
        Нам нужно обновить компонент <code>HomePage</code> с помощью следующего
        кода:
  ...
  return (
    <div>
      ...
      <p>
        ...
        начать повторно реализовывать фильтры, которые мы убрали на первом шаге.
        Фильтры теперь будут храниться в состоянии в <code>HomePage</code> и
        передаваться в функцию <code>fetchListings</code>. Это упрощает нашу
        задачу, потому что нам больше не нужно беспокоиться о фильтрации списков
        самостоятельно. Макет API позаботится об этом за нас, и все, что нам
        нужно сделать, это передать ему необходимые данные.
      </p>
      <p>
        Нам нужно создать новую переменную состояния под названием{' '}
        <code>filters</code>. Это будет объект с тремя свойствами, которые мы
        имели ранее: <code>dates</code>, <code>guests</code> и{' '}
        <code>search</code>. Мы установим начальное состояние на{' '}
        <code>undefined</code> для <code>dates</code>, <code>0</code> для{' '}
        <code>guests</code> и пустую строку для <code>search</code>. Затем мы
        передадим эти фильтры в нашу функцию <code>fetchListings</code> в
        качестве параметров.
      </p>
      <p>
        ...
        зависимостей. Это приведет к тому, что функция{' '}
        <code>fetchListings</code>
        будет повторно запускаться каждый раз, когда <code>filters</code>{' '}
        изменяется, что нам и нужно.
      </p>
      <p>
        ...
        <code>ListingFilters</code>, в состояние <code>HomePage</code>.
        Поскольку мы добавили <code>filters</code> в массив зависимостей нашего{' '}
        <code>useEffect</code>, функция <code>fetchListings</code> будет
        автоматически вызываться всякий раз, когда они изменяются.
      </p>
      <p>
        Нам нужно обновить компонент <code>HomePage</code> с помощью следующего
        кода:
  ...
  return (
    <div>
      ...
      <p>
        ...
        недостающий элемент. Давайте подумаем о другом сценарии, который может
        произойти. В данный момент, когда компонент <code>HomePage</code>
      ...
      <p>
        ...
        одновременно выполняться 2 запроса, что не хорошо. Еще хуже, если первый
        запрос задержится по какой-либо причине и вернется после второго, он
        перезапишет результаты второго, что приведет к несоответствию интерфейса
        с данными. Это называется состоянием гонки, и нам нужно его
        предотвратить.
      </p>
      <p>
        Мы предотвратим это, убедившись, что любой незавершенный запрос
        отменяется перед тем, как будет запущен новый. Это гарантирует, что у
        нас всегда будет только один запрос в полете и что мы всегда будем
        видеть самые актуальные результаты.
      </p>
      <p>
        Для этого мы будем использовать <code>AbortController</code>, который мы
        можем использовать для отмены запроса. Нам нужно будет хранить это в
        компоненте. Однако, поскольку мы не показываем контроллер нигде в UI, мы
        можем сохранить его в рефе через хук <code>useRef</code>, чтобы избежать
        повторного рендеринга нашего компонента каждый раз, когда он изменяется.
      </p>
      <p>
      ...
      <p>
        ...
        <code>isCancel</code> для ошибки перед установкой состояния. Если ошибка
        вызвана отменой, мы игнорируем ее.
      </p>
      <p>
        Нам нужно обновить компонент <code>HomePage</code> с помощью следующего
        кода:
...
export const Step7 = () => {
  return (
    <div>
      ...
      <p>
        Мы собираемся добавить больше деталей в компонент{' '}
      ...
      <p>
        ...
        использовать новые иконки из <code>lucide-react</code>. С их помощью мы
        добавим больше деталей в <code>CardContent</code> компонента{' '}
        <code>ListingCard</code>.
      </p>
      <p>
...
export const Step8 = () => {
  return (
    <div>
      ...
      <p>
        Следующее, что нам нужно сделать, это улучшить отображение изображений
        списка. В настоящее время отображается только первое изображение, а
        остальные игнорируются. Мы собираемся изменить это, добавив красивую
        карусель изображений в компонент <code>ListingCard</code>.
      </p>
      <p>
      ...
      <p>
        Этот компонент будет принимать <code>listing</code> в качестве пропса и
        использовать его для перебора каждого изображения и отображения
        карусели. Мы будем использовать тот же тег <code>img</code>, что и ранее
        для каждого элемента, но интегрируем его в компонент{' '}
      ...
      <p>
        ...
        <code>CarouselPrevious</code> и <code>CarouselNext</code>. Все они будут
        работать вместе, потому что <code>Carousel</code> является составным
        компонентом, и каждый из этих компонентов принадлежит ему.
      </p>
      <p>
        ...
        наводит курсор на карусель. Таким образом, мы сохраняем чистоту нашего
        интерфейса, не теряя функциональности. Для этого нам нужно создать новую
        переменную состояния, называемую <code>isHovering</code>, и обновить её,
        используя обработчики событий <code>onMouseEnter</code>и{' '}
        <code>onMouseLeave</code> на компоненте <code>Carousel</code>.
      </p>
      <p>
        Нам нужно создать новый файл в директории <code>src/components</code> с
        именем <code>ListingCardImages.jsx</code> со следующим кодом:
      </p>
...
export const Step9 = () => {
  return (
    <div>
      <h2>
        Обновление <code>ListingCard</code> с помощью{' '}
        <code>ListingCardImages</code>
      </h2>
      <p>
        Последнее, что нам нужно сделать, это добавить наш новый{' '}
        <code>ListingCardImages</code> компонент в <code>ListingCard</code>,
        чтобы показать наш карусель изображений. Мы сделаем это, импортировав{' '}
      ...
      <p>
        ...
        для первого изображения, и будем использовать компонент{' '}
        <code>ListingCardImages</code>
        вместо него. Нам нужно будет передать ему <code>listing</code> в
        качестве пропсов, чтобы он работал правильно.
      </p>
      <p>
