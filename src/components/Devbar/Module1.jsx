...
const tasks = [
  'Создайте компонент <code>HomePage</code>',
  'Добавьте <code>HomePage</code> в <code>App</code>',
  'Создайте компонент <code>ListingList</code>',
  'Создайте компонент <code>ListingCard</code>',
  'Обновите <code>ListingList</code> с <code>ListingCard</code>',
  'Обновите <code>HomePage</code> с <code>ListingList</code>',
];
...
export const Step1 = () => {
  return (
    <div>
      ...
      <p>
        Давайте начнем с создания домашней страницы. Эта страница важна, потому
        что это первая страница, на которую попадут пользователи, когда откроют
        наше приложение. Для этого нам нужно создать наш первый компонент
        страницы — <code>HomePage</code>.
      </p>
      <p>
        Компоненты страниц размещаются в каталоге <code>src/pages</code>. Здесь
        будут храниться все наши страницы. <code>HomePage</code> будет отвечать
        за рендеринг списка объявлений, но пока она просто будет рендерить
        простой <code>div</code> с некоторыми стилями и сообщением "Hello
        World!".
      </p>
      <p>
        Мы применим несколько базовых стилей к нашему компоненту, таких как{' '}
        <code>container</code> и немного отступов. По мере прохождения курса мы
        будем применять эти стили ко всем компонентам страниц, которые создадим,
        чтобы обеспечить единообразный интерфейс в нашем приложении.
      </p>
      <p>
        Нам нужно создать новый файл в каталоге <code>src/pages</code> с именем{' '}
        <code>HomePage.jsx</code> со следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        который рендерится при загрузке нашего приложения. В настоящее время это
        корень нашего приложения, и здесь также находится компонент{' '}
        <code>Devbar</code>.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        Этот компонент будет отвечать за рендеринг списка объявлений. Это те
        объявления, которые смогут просматривать наши пользователи. Пока что,
        так как у нас нет никаких объявлений для показа, мы просто отобразим
        сообщение "Здесь будут объявления!" в нашем компоненте.
      </p>
      <p>
        ...
        фичи, он будет находиться в корне папки <code>src/components</code>,
        рядом с нашими UI-компонентами в папке <code>ui</code>.
      </p>
      <p>
        Нам нужно создать новый файл в каталоге <code>src/components</code> с
        именем <code>ListingList.jsx</code> со следующим кодом:
      </p>
...
export const Step4 = () => {
  return (
    <div>
      ...
      <p>
        Этот компонент также будет компонентом фичи, что означает, что он тоже
        будет находиться в корне папки <code>src/components</code>, рядом с
        компонентом <code>ListingList</code>, который мы создали на предыдущем
        шаге.
      </p>
      <p>
        ...
        <code>props</code>. Затем мы сможем использовать это{' '}
        <code>listing</code>в этом компоненте для рендеринга главного
        изображения и названия объявления.
      </p>
      <p>
        Для изображения мы будем использовать функцию <code>getImageUrl</code>
        из файла утилит изображений в папке <code>src/utils</code>. Эта функция
        предоставит нам URL для изображения, который мы можем использовать для
        его отображения. У каждого объявления есть несколько изображений, но
      ...
      <p>
        Нам нужно создать новый файл в каталоге <code>src/components</code> с
        именем <code>ListingCard.jsx</code> со следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        , поэтому нам нужно обновить наш компонент <code>ListingList</code>,
        чтобы он принимал <code>listings</code> в качестве <code>props</code>.
      </p>
      <p>
        Также нам нужно будет проверить длину <code>listings</code>, так как
        может случиться так, что мы получим пустой список. Если нет объявлений
        для отображения, мы отобразим простое сообщение "Объявлений не найдено."
        В противном случае мы пройдемся по <code>listings</code> и отобразим{' '}
        <code>ListingCard</code> для каждого, передав текущее
  ...
  return (
    <div>
      ...
      <p>
        ...
        "Объявления здесь!" в<code>HomePage</code> на компонент{' '}
        <code>ListingList</code> и передать ему некоторые <code>listings</code>{' '}
        через <code>props</code>.
      </p>
      <p>
        ...
        <code>HomePage</code> и передать в <code>ListingList</code> для
        отображения. На данный момент это будет статический список объявлений. В
        последующих модулях мы обновим это, чтобы он был более динамичным и
        фактически извлекал данные из базы данных
        <code>localStorage</code>.
      </p>
      <p>
        Одно, на что стоит обратить внимание, это то, что мы могли бы
        импортировать объявления прямо внутри <code>ListingList</code>, не
        передавая их через <code>HomePage</code>. Однако лучше оставить их
        здесь, потому что ответственность за <code>listings</code> должна лежать
        на <code>HomePage</code>. <code>ListingList</code> будет отвечать за их
        отображение через <code>ListingCard</code>, а <code>ListingCard</code>{' '}
        фактически будет отображать каждое объявление.
      </p>
      <p>
  ...
  return (
    <div className='relative'>
      ...
      <p>
        ...
        необходимое, чтобы сделать наше приложение динамичным, добавив
        состояние!
      </p>
      <p>
