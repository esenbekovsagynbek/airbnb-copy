  ...
  return (
    <div>
      ...
      <p>
        ...
        настроим React Hook Form для легкой реализации форм в нашем приложении!
      </p>
      <h3>Описание</h3>
      <Separator className='mb-2' />
      <p>
      ...
      <p>
        ...
        также добавим кнопку для выхода, которая должна будет выйти из системы и
        перенаправить пользователя на страницу входа.
      </p>
      <p>
        Нам также нужно будет обрабатывать обновление токена доступа
        пользователя. Поскольку мы храним его только в памяти, он будет теряться
        каждый раз, когда пользователь обновляет страницу. Мы реализуем базовую
        версию обновления и доступа токенов. Это будет самой крупной функцией
        нашего приложения на данный момент, так что готовьтесь!
      </p>
      <h3>Задачи</h3>
      <Separator className='mb-2' />
      <ul>
  ...
  return (
    <div>
      ...
      <p>
        ...
        состояния для хранения <code>токена</code> и функцию для его обновления.
      </p>
      <p>
        Мы установим <code>токен</code> по умолчанию на <code>undefined</code>
        при первоначальной монтировке. Позже мы будем использовать это
        состояние, чтобы определить, был ли <code>токен</code> получен или нет.
        Это будет полезно для отображения состояния ожидания пользовательского
        интерфейса, пока <code>токен</code> загружается, и для определения,
        вошел ли пользователь в систему после его получения.
      </p>
      <p>
      ...
      <p>
        Нам нужно будет создать новый файл в папке <code>src/components</code> с
        названием <code>AuthProvider.jsx</code> со следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>токену</code> откуда угодно. <code>Router</code> содержит каждый
        компонент, который у нас будет в приложении.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        приложению, следующим шагом будет получение <code>токена</code> из API и
        сохранение его в провайдере, чтобы он больше не был{' '}
        <code>undefined</code>. Мы будем использовать конечную точку{' '}
        <code>/api/me</code> для этого, которая вернет <code>токен</code>, если
        пользователь вошел в систему. Это позволит нам определить состояние
        входа пользователя сразу после монтирования приложения.
      </p>
      <p>
        Мы создадим <code>useEffect</code> с функцией <code>fetchMe</code> для
        получения <code>токена</code>. Если запрос будет успешным, мы обновим
        состояние <code>токена</code> полученным{' '}
        <code>response.data.accessToken</code>. Если запрос завершится неудачно,
        мы установим состояние <code>токена</code> в <code>null</code>. Мы
        специально устанавливаем его в <code>null</code> вместо{' '}
        <code>undefined</code>, чтобы различать два состояния. <code>null</code>{' '}
        будет означать, что мы его получили, но он не был доступен, а{' '}
        <code>undefined</code> будет означать, что он еще не был получен.
      </p>
      <p>
...
export const Step4 = () => {
  return (
    <div>
      ...
      <p>
        Теперь, когда <code>токен</code> получен и доступен в нашем приложении,
        мы можем начать его использовать. Первое, что мы хотим сделать, — это
        скрыть компонент <code>Navbar</code>, если пользователь не вошел в
        систему. Это подготовит нас к тому, что мы будем показывать экран входа,
        который не должен иметь навигационную панель.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        содержимое посередине страницы. Это подготовительная работа для создания
        компонента формы входа, который будет довольно маленьким по размеру и
  ...
  return (
    <div>
      ...
      <p>
        ...
        компонент будет отвечать за отображение формы входа, обработку валидации
        и отправку данных формы на API, чтобы пользователь мог войти в наше
        приложение.
      </p>
      <p>
        <code>SignInForm</code> будет компонентом функционала, который будет
        обрабатывать все, что связано с функцией входа пользователя. Мы сможем
        использовать этот компонент на любой странице или разделе страницы, где
        мы хотим иметь функцию входа.
      </p>
      <p>
        Мы будем использовать React Hook Form для обработки валидации и отправки
        формы, а также библиотеку <code>zod</code> для определения схемы нашей
        формы и использования ее для валидации формы. Мы сможем внедрить схему{' '}
        <code>zod</code> в React Hook Form через функцию{' '}
        <code>zodResolver</code>.
      </p>
      <p>
        Мы оставим всё простым, включив только поля <code>email</code> и{' '}
        <code>password</code>. Мы используем функцию <code>z.email()</code> для
        того, чтобы убедиться, что принимаются только действительные адреса
        электронной почты. Для пароля мы просто ограничим его минимум 8
        символами.
      </p>
      <p>
        Мы создадим новый файл в папке <code>src/components</code> с названием{' '}
        <code>SignInForm.jsx</code> со следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        отправку формы, когда пользователь пытается войти в систему. Что нам
        нужно сделать, так это отправить запрос на сервер с данными{' '}
        <code>email</code> и <code>password</code>, чтобы сервер мог проверить
      ...
      <p>
        ...
        <code>AuthProvider</code> через <code>useAuth</code>, чтобы сохранить
        токен в провайдере и сделать его доступным в нашем приложении.
      </p>
      <p>
        ...
        <code>isSubmitting</code>, а также к свойству <code>errors</code>,
        которое мы создали ранее. Мы можем использовать их, чтобы определить, в
        каком состоянии находится форма.
      </p>
      <p>
        ...
        использовать функцию <code>setError</code> из React Hook Form, чтобы
        установить "корневую" ошибку. Мы установим ее под <code>root</code>. Это
        затем будет доступно через <code>errors.root</code>, что мы можем
...
export const Step10 = () => {
  return (
    <div>
      ...
      <p>
        В настоящее время функции аутентификации отключены в нашем приложении.
        Даже с действительными учетными данными вход в систему не приведет к
        фактическому входу пользователя. Это было сделано, чтобы мы могли
        строить приложение до этого момента, не беспокоясь об аутентификации.
      </p>
      <p>
        Однако, чтобы мы могли продолжить, нам нужно включить ее. Для этого
        просто измените переменную окружения <code>VITE_USE_AUTH</code> и
        установите ее в <code>true</code>. Это включит аутентификацию и
        заблокирует все запросы к приложению, если пользователь не вошел в
        систему. В таком случае приложение в основном перестанет работать, но мы
        это исправим в следующих шагах!
      </p>
      <p>
        С включенной аутентификацией теперь мы можем войти в приложение,
        используя доступную демонстрационную учетную запись. Электронная почта{' '}
        <code>demo@test.io</code>, а пароль <code>password12345</code>. Мы можем
        войти, увидеть появление <code>Navbar</code> и даже попробовать перейти
        на страницу "Домашняя"! Но поскольку наш <code>token</code> еще не
        отправляется в любых запросах, мы не увидим никаких данных.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>token</code> пользователя ко всем запросам. На данный момент, даже
        если пользователь вошел в систему с правильными учетными данными и{' '}
        <code>token</code> хранится в компоненте <code>AuthProvider</code>, мы
        не отправляем этот токен в никаких запросах, поэтому сервер будет
        считать пользователя неавторизованным.
      </p>
      <p>
        Чтобы это исправить, нам нужно будет отправить его в каждом запросе,
        используя перехватчики из <code>axios</code>. Перехватчики позволяют нам
        перехватывать запросы перед их отправкой и ответы перед их возвратом. Мы
        будем использовать это, чтобы добавить токен ко всем нашим запросам,
        когда они отправляются.
      </p>
      <p>
        Самое логичное место для добавления этого кода — это внутри{' '}
        <code>AuthProvider</code>, так как именно там хранится{' '}
        <code>token</code>. Мы хотим зарегистрировать перехватчик при
        монтировании и каждый раз, когда <code>token</code> изменяется. Это
        обеспечит синхронизацию последнего токена со всеми нашими запросами.
      </p>
      <p>
        Для этого мы будем использовать <code>useLayoutEffect</code>, так как
        это предотвратит дальнейшую отрисовку до выполнения кода внутри. Это
...
export const Step12 = () => {
  return (
    <div>
      ...
      <p>
        На предыдущем шаге мы добавили <code>token</code> ко всем запросам. В
        целях безопасности этот токен действителен только в течение 15 минут.
        После этого нам нужно будет обновить его от имени пользователя. Это в
        основном обрабатывается на сервере, но нам также нужно сделать несколько
        вещей на стороне клиента, чтобы это завершить.
      </p>
      <p>
        Если учетные данные пользователя действительны, но срок действия токена
        доступа истек, сервер вернет ответ с кодом состояния <code>403</code>, а
        также сообщение <code>Unauthorized</code>. Если это произойдет, нам
        нужно попробовать обновить токен, сделав запрос к{' '}
        <code>/api/refreshToken</code>. Если <code>refreshToken</code>{' '}
        пользователя все еще действителен, мы получим новый токен доступа{' '}
        <code>token</code>, который мы сможем использовать вместо истекшего. Мы
        сможем сделать это до тех пор, пока <code>refreshToken</code> от сервера
        действителен.
      </p>
      <p>
      ...
      <p>
        Нам нужно будет создать еще один перехватчик <code>axios</code> для
        этого, но на этот раз для ответа. Перехватчик будет проверять код
        состояния
      ...
      <p>
        Если новый <code>token</code> возвращается, мы установим его в
        состоянии, а также в заголовках <code>originalRequest</code>. Нам также
        нужно будет установить пользовательскую переменную <code>_retry</code>,
        которую мы можем использовать в перехватчике запроса, чтобы
        предотвратить переопределение этого нового <code>token</code>{' '}
        перехватчиком запроса. Наконец, мы вызовем <code>originalRequest</code>{' '}
        и вернем его ответ.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        который будет перенаправлять пользователя на <code>SignInPage</code>,
        если он не вошел в систему. Мы обернем каждый из наших маршрутов в нашем{' '}
        <code>Router</code> с этим компонентом, чтобы, независимо от того, на
        какой странице находится пользователь, если он не вошел в систему, его
        перенаправят на <code>SignInPage</code>, и он не сможет получить доступ
        к другим страницам.
      </p>
      <p>
        ...
        использовать функцию <code>navigate</code> для перенаправления
        пользователя и передадим <code>replace</code> как <code>true</code>,
        чтобы очистить историю и предотвратить возможность возврата пользователя
        назад.
      </p>
      <p>
        ...
        недействительным. Поэтому нам нужно будет проверить на <code>null</code>{' '}
        в частности и перенаправлять на основе этого.
      </p>
      <p>
        Нам нужно будет создать новый файл в папке <code>src/components</code> с
        именем <code>Route.jsx</code> и следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        обновить все наши маршруты, чтобы использовать его. Мы обернем каждый из
        наших маршрутов в нашем <code>Router</code> и передадим{' '}
        <code>isProtected</code> тем маршрутам, которые мы хотим защитить для
        только вошедших пользователей.
      </p>
      <p>
        Поскольку мы поместили всю эту функциональность в один переиспользуемый
        компонент, если в будущем мы захотим добавить какую-либо дополнительную
        функциональность к нашим маршрутам, мы сможем легко сделать это через
        этот компонент <code>Route</code> и применить к всем маршрутам. Вот в
        чем сила React!
      </p>
      <p>
        Нам нужно будет обновить компонент <code>Router</code> с следующим
        кодом:
      </p>
      <CodeHighlighter
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>onSubmit</code>, то ничего не будет мешать пользователю вручную
        изменить URL браузера на <code>/signin</code> и получить доступ к
        странице.
      </p>
      <p>
      ...
      <p>
        ...
        <code>useNavigate</code> из <code>react-router-dom</code>. Мы используем
        этот хук, чтобы получить функцию <code>navigate</code>, а затем
        используем хук <code>useEffect</code>, чтобы следить за изменениями
        состояния <code>token</code>. Если <code>token</code> имеет истинное
        значение, мы перенаправим пользователя на домашнюю страницу. Мы также
        используем параметр <code>replace</code>, чтобы удалить маршрут{' '}
        <code>/signin</code> из истории, чтобы пользователь не мог вернуться
        назад к нему.
      </p>
      <p>
        Это довольно похоже на то, что мы сделали с компонентом{' '}
        <code>Route</code>, но в обратном направлении. Мы могли бы добавить эту
        функциональность внутрь <code>Route</code>, но в данный момент у нас
        есть только одна такая страница, так что нормально поместить ее сюда.
      </p>
      <p>
        Нам нужно будет обновить компонент <code>SignInPage</code> с следующим
        кодом:
      </p>
      <CodeHighlighter
...
export const Step16 = () => {
  return (
    <div>
      ...
      <p>
        Последний шаг в нашей аутентификации — это возможность выхода
        пользователя из системы. Мы сделаем это, добавив кнопку в компонент{' '}
        <code>Navbar</code>, которая будет вызывать конечную точку{' '}
        <code>/api/signout</code> и выводить пользователя из системы.
      </p>
      <p>
        Единственное, что нам нужно сделать здесь, — это установить{' '}
        <code>token</code>в <code>null</code> после успешного ответа на выход,
        чтобы приложение знало, что пользователь вышел из системы. Установив{' '}
        <code>token</code> в <code>null</code>, приложение справится с
        остальным, поскольку мы настроили необходимые компоненты для этого.
      </p>
      <p>
        Мы добавим новый элемент в <code>Navbar</code> для кнопки выхода. Мы
        создадим выпадающее меню под названием "Аккаунт", в котором будет кнопка
        выхода. Это позволит нам в будущем добавлять больше элементов в это
        меню, если потребуется. Мы также обновим некоторые стили в{' '}
        <code>Navbar</code>, чтобы сделать его более привлекательным.
      </p>
      <p>
        Нам нужно будет обновить компонент <code>Navbar</code> с следующим
        кодом:
      </p>
      <CodeHighlighter
        highlightedLines={[
  ...
  return (
    <div className='relative'>
      ...
      <p>
        ...
        входить в систему, у них есть правильная валидация в форме входа, и они
        могут получать доступ только к защищённым частям приложения, если они
        вошли в систему.
      </p>
      <p>
      ...
      <p>
        ...
        <code>8-deploying</code> в выпадающем меню выше. Увидимся там!
      </p>
