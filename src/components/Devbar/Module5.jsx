  ...
  return (
    <div>
      ...
      <p>
        ...
        обрабатываем наши состояния ошибок и загрузки одинаковым образом, и даже
        создали одинаковые функции рендеринга, которые отображают наши данные в
        обоих компонентах! Пора оптимизировать и рефакторить.
      </p>
      <p>
...
export const Step1 = () => {
  return (
    <div>
      ...
      <p>
        Наш код становится немного запутанным. У нас есть повторяющийся код в
        двух местах: в <code>HomePage</code> и в <code>ListingDetailsPage</code>
        . Оба компонента получают данные, обрабатывают состояния загрузки и
        ошибок, и в конце концов отображают данные с помощью кастомной функции
        одинаковым образом. Это отличное применение для создания кастомного
        переиспользуемого хука!
      </p>
      <p>
        Чтобы улучшить ситуацию, мы можем создать кастомный хук для обработки
        всех операций по получению данных, который мы сможем повторно
        использовать в обоих компонентах. Мы назовем его <code>useFetch</code> и
        передадим ему <code>url</code> для получения данных и некоторые
        необязательные <code>options</code> для передачи в <code>api</code>.
        Этот хук будет иметь такую же функциональность, которую мы сейчас
        используем в обоих компонентах.
      </p>
      <p>
        Наш хук <code>useFetch</code> будет содержать <code>useEffect</code>,
        который будет получать данные на основе предоставленного{' '}
        <code>url</code>, будет управлять своими состояниями загрузки и ошибок,
        обрабатывать гонки с помощью <code>AbortController</code> и, наконец,
        возвращать объект с 3 свойствами: <code>data</code>, <code>error</code>{' '}
        и <code>isLoading</code> для использования в компоненте.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        так как они теперь будут обрабатываться нашим кастомным хуком. Также мы
        удалим состояние <code>listings</code> и <code>useEffect</code>, который
        загружал наши объявления, поскольку они больше не нужны.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        ввели ошибку. Объявления теперь загружаются снова и снова в бесконечном
        цикле, и это не хорошо!
      </p>
      <p>
      ...
      <p>
        ...
        React, который позволит нам создать переменную, которая будет одинаковой
        на каждой перерисовке, если мы не захотим, чтобы она изменилась. Это
        называется стабильной ссылкой. Затем мы можем передать эту переменную в{' '}
        <code>useFetch</code> вместо <code>options</code>
        напрямую.
      </p>
      <p>
        Мы можем просто создать новую переменную <code>fetchOptions</code> с тем
        же объектом, что и ранее, но теперь мы обернем его в{' '}
        ...
        <code>fetchOptions</code> стабильной между перерисовками, за исключением
        случаев, когда <code>filters</code> изменяется.
      </p>
      <p>
...
export const Step4 = () => {
  return (
    <div>
      ...
      <p>
        Поскольку мы рефакторим компонент <code>HomePage</code>, давайте внесем
        еще одно небольшое улучшение в наш код. Прямо сейчас, каждый раз, когда{' '}
        <code>HomePage</code> перерисовывается, компонент{' '}
      ...
      <p>
        ...
        перерисовке, поскольку она не является стабильной ссылкой. Это также
        приведет к перерисовке <code>ListingFilters</code>, поскольку его пропсы
        изменились.
      </p>
      <p>
        Поскольку <code>ListingFilters</code> не интересует ничего из{' '}
        <code>HomePage</code>, кроме функции обратного вызова{' '}
        <code>onChange</code>, которую она получает, мы должны предотвратить ее
        перерисовку, когда <code>HomePage</code> перерисовывается. Это поможет
        нам сэкономить один цикл рендеринга, и если в будущем мы добавим больше
        функциональности в<code>HomePage</code>, мы гарантированно не повлияем
        на производительность из-за избыточной перерисовки{' '}
      ...
      <p>
        ...
        функцию стабильной ссылкой, и она больше не будет вызывать перерисовку,
        если массив зависимостей не изменится. И поскольку в данный момент она
        не зависит ни от чего, этот массив зависимостей будет пустым.
      </p>
      <p>
        Нам нужно обновить компонент <code>HomePage</code> следующим кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        тогда, когда его пропсы изменились. Если мы этого не сделаем, даже с{' '}
        <code>useCallback</code> он все равно будет перерисовываться, потому что
        React по умолчанию не проверяет пропсы компонента перед тем, как решить,
        нужно ли его перерисовывать.
      </p>
      <p>
        Единственное, что нам нужно сделать, чтобы это исправить, —
        импортировать <code>memo</code> из <code>React</code>, а в конце файла,
        где мы экспортируем компонент, обернуть экспорт в <code>memo</code>. Это
      ...
      <p>
        Нам нужно обновить компонент <code>ListingFilters</code> следующим
        кодом:
      </p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        самое с компонентом <code>ListingDetailsPage</code>. Нам нужно
        избавиться от состояний <code>error</code> и <code>isLoading</code>, а
        также от состояния <code>listing</code> и <code>useEffect</code>,
        который извлекает наш листинг. Всё будет обрабатываться с помощью
        пользовательского хука снова.
      </p>
      <p>
        На этот раз нам не нужно передавать никаких опций в{' '}
        <code>useFetch</code> так как всё, что нам нужно, это{' '}
        <code>listingId</code>, и он передается напрямую через <code>url</code>.
        Это также означает, что нам не нужно использовать <code>useMemo</code>{' '}
        здесь. А так как этот компонент не передает ничего своим детям, нам
        также не нужно беспокоиться о <code>useCallback</code>.
      </p>
      <p>
...
export const Step7 = () => {
  return (
    <div>
      ...
      <p>
        Давайте теперь сделаем еще один рефакторинг нашего кода. Каждый раз,
        когда мы извлекаем данные для отображения в <code>HomePage</code> и{' '}
        <code>ListingDetailsPage</code>, мы вручную создаем вспомогательную
        функцию для отображения пользовательского интерфейса для состояний
        загрузки и ошибок. Это здорово для пользовательского опыта, но в итоге
        мы снова повторяем одну и ту же логику в обоих компонентах. Так же, как
        мы сделали для извлечения данных, давайте теперь рефакторим это, чтобы
        создать переиспользуемый компонент, который мы сможем использовать на
        обеих страницах для отображения нашего интерфейса.
      </p>
      <p>
        Создание пользовательского компонента для этого — хорошая идея, потому
        что это шаблон, который мы, вероятно, будем использовать в нескольких
        местах по мере роста приложения. Каждый раз, когда вы обнаруживаете, что
        повторяете код подобным образом, всегда полезно задать себе вопрос,
      ...
      <p>
        ...
        который будет принимать 3 пропса: <code>children</code>,{' '}
        <code>error</code> и <code>isLoading</code>. Затем мы можем поместить
        нашу логику обработки ошибок и загрузки туда, так же как мы сделали в
        наших пользовательских функциях рендеринга.
      </p>
      <p>
        Нам нужно создать новый файл внутри <code>components</code> под
        названием <code>DataRenderer.jsx</code> с следующим кодом:
      </p>
...
export const Step8 = () => {
  return (
    <div>
      <h2>
        Обновление <code>HomePage</code> с использованием{' '}
        <code>DataRenderer</code>
      </h2>
      <p>
        Теперь, когда у нас есть наш компонент <code>DataRenderer</code>, мы
        можем обновить <code>HomePage</code> с его помощью и избавиться от
        функции рендеринга, поскольку наш новый компонент теперь будет
        обрабатывать это. Все, что нам нужно сделать, это передать наши
        состояния <code>error</code> и <code>isLoading</code> ему, и он
        автоматически справится с остальным!
      </p>
      <p>
        Преимущество такого подхода в том, что теперь наш компонент{' '}
        <code>HomePage</code> действительно стал маленьким и простым по
        сравнению с тем, каким он был раньше. Он стал гораздо более управляемым
        и легким для работы. Все, что он делает, это хранит хук извлечения
        данных <code>useFetch</code> вместе с фильтрами и делегирует все
        остальное другим компонентам.
      </p>
      <p>
...
export const Step9 = () => {
  return (
    <div>
      <h2>
        Обновление <code>ListingDetailsPage</code> с использованием{' '}
        <code>DataRenderer</code>
      </h2>
      <p>
        Так же, как мы сделали с <code>HomePage</code>, мы теперь сделаем и с{' '}
        <code>ListingDetailsPage</code>. Мы избавимся от пользовательской
        функции рендеринга и просто используем <code>DataRenderer</code>,
        передавая ему состояния <code>error</code> и <code>isLoading</code>.
      </p>
      <p>
        ...
        выглядеть большинство ваших компонентов в React-приложении! Наличие
        таких маленьких компонентов хорошо масштабируется и упрощает работу с
        вашим кодом со временем.
      </p>
      <p>
  ...
  return (
    <div>
      ...
      <p>
        ...
        <code>useFetch</code>. На данный момент, каждый раз, когда мы переходим
        на <code>HomePage</code> или <code>ListingDetailsPage</code>, мы
        запрашиваем данные с API, и видим индикатор загрузки. Это раздражает,
        потому что происходит даже тогда, когда мы переходим на любой экран,
        даже если мы только что получили данные несколько секунд назад.
      </p>
      <p>
        В идеале, мы должны сохранять полученные данные в кеш и устанавливать
        порог времени, по истечении которого мы будем считать данные
        устаревшими, и только тогда, когда этот порог будет превышен, мы снова
        запрашиваем данные. Таким образом, если мы уже недавно получили данные,
        мы можем сразу показать их из кеша и улучшить пользовательский опыт.
      </p>
      <p>
        Для этого мы будем использовать <code>localStorage</code> в качестве
        нашего кеша и хранить данные там. Мы также сохраним переменную{' '}
        <code>lastFetched</code>, которая позволит нам проверить, как давно были
        получены данные. Наконец, мы используем утилиты{' '}
        <code>localStorage</code> для реализации кеширования в{' '}
        <code>useFetch</code>.
      </p>
      <p>
...
export const Completed = () => {
  return (
    <div className='relative'>
      ...
      <p>
        Поздравляем! Вы успешно завершили 5-й модуль курса. Приложение теперь
        должно работать так же, как и раньше, но с несколькими улучшениями. У
        вас теперь есть полностью работающий кеш для получения данных, который
        основан на URL и параметрах, и код должен быть гораздо проще, удобнее
      ...
      <p>
        ...
        производительность с помощью <code>useMemo</code> и{' '}
        <code>useCallback</code>, мы научились создавать пользовательские хуки,
        такие как <code>useFetch</code>, и даже извлекать общую функциональность
        в пользовательские компоненты, такие как <code>DataRenderer</code>!
      </p>
      <p>
